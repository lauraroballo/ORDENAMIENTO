...
public class Ingresar_Aleat extends javax.swing.JFrame {
 ...
 private GeneradorAleatorio generador = new GeneradorAleatorio();
   ...
   int cantidadAleatoria = Integer.parseInt(jAlt1.getText());
    ...
    // Verificación para asegurarse de que el array no es nulo
    if (numerosGenerados != null) {
        arregloEstructura.setNumerosAleatorios(numerosGenerados);
        arregloEstructura.ordenarNumeros();

        // Inicializar los arreglos antes de obtener los resultados
        seleccionOrdenado = arregloEstructura.getSeleccionOrdenado();
        burbujaOrdenado = new int[cantidadAleatoria];
        insercionOrdenado = new int[cantidadAleatoria];
        
        // Obtener los resultados ordenados
        burbujaOrdenado = arregloEstructura.getBurbujaOrdenado();
        insercionOrdenado = arregloEstructura.getInsercionOrdenado();
        
        shellOrdenado = arregloEstructura.getShellOrdenado();
        heapSortOrdenado = arregloEstructura.getHeapSortOrdenado();
        quickSortOrdenado = arregloEstructura.getQuickSortOrdenado();

        // Crear e instanciar la ventana Resultado pasando el arreglo
